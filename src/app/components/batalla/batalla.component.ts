...
  mensajeModal: string = '';
    // PROPIEDAD AÑADIDA para controlar el botón del modal
...
          this.duelosGanados = (this.partida as any).duelosGanados || 0;
          ...
          // ** LÓGICA MODIFICADA PARA EL ÍTEM INICIAL **
          // SOLO generamos un item si es la primera batalla (duelosGanados es 0)
          if (this.duelosGanados === 0) {
              ...
              this.pokeapi.getItemsById(idNuevoItem).subscribe({
                  next: async (nuevoItem: Items) => {
                      ...
                      console.log('Nuevo item random agregado y traducido (Primera Batalla):', nuevoItem.localizedName);
    
                      // Actualizamos logs
                      console.log('Ítems totales:', this.jugador!.items!.length);
    
                      // INICIAR BATALLA (dentro del subscribe de item para asegurar que cargó)
                      this.iniciarBatalla();
                  },
                  error: (err: Error) => {
              ...
              // Si ya hay duelos ganados, simplemente iniciamos la batalla
              console.log('Duelos ganados > 0. No se agrega ítem inicial. Iniciando batalla.');
              this.iniciarBatalla();
          }
...
    // Usamos el valor ya calculado en iniciarBatalla o en cerrarModal
...
    // [CAMBIO 1] Asignar el resultado de la victoria/derrota
    this.esVictoria = ganador;
...
    if (ganador) {
      // [CAMBIO 2] Lógica de puntuación: 3 puntos si es JEFE, 1 punto si es normal.
...
      });
      ...
      // ** INICIO DE LÓGICA MODIFICADA PARA DAR ÍTEM POR VICTORIA **
...
          this.jugador?.items?.push(value);
          console.log(`Item de Victoria agregado (${this.isBossBattle ? 'BOSS' : 'NORMAL'}): ${value.localizedName}`);

          // Lógica para revivir Pokémon debilitados y restaurar HP/Stats
...
              this.partida!.puntuacion = puntajeNuevo;
              this.partida!.duelosGanados = this.duelosGanados;
              // Continuamos la batalla SOLAMENTE después de actualizar la BD y obtener el ítem
              this.continuarBatalla();
            },
            error: (error: Error) => {
              console.error(this.translate.instant('log.scoreUpdateError'), error);
              // Aunque falle la BD, intentamos seguir el juego
              this.continuarBatalla();
            }
          });
...
        },
        error: (err: Error) => {
          console.log('Error al obtener el ítem de victoria:', err)
          
          // FALLBACK si falla la obtención del ítem: asegurar que el juego continúe
          for (let i = 0; i < this.pokemonDebilitados.length; i++) {
            this.jugador?.equipo.push(this.pokemonDebilitados[i]);
          }
          this.jugador!.equipo.forEach(p => {
...
        },
      });
      // ** FIN DE LÓGICA MODIFICADA PARA DAR ÍTEM POR VICTORIA **
